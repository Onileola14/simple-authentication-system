...
const getSingleUser = async (req, res) => {
    ...
    const user = await User_1.User.findOne({ _id: userId }).select("-password");
    if (!user) {
...
const deleteUser = async (req, res) => {
    const { id: userId } = req.params;
    (0, checkPermission_1.createPermission)(req.user, userId);
    const user = await User_1.User.findOneAndDelete({ _id: userId });
    if (!user) {
        throw new NotFoundError(`No user with id : ${userId}`);
    }
...
const updateUser = async (req, res) => {
    ...
    if (email !== undefined)
        updates.email = email;
    const user = await User_1.User.findOneAndUpdate({ _id: userId }, updates, {
        new: true,
        runValidators: true,
    }).select("-password");
    if (!user) {
